...
# Initialize Adafruit IO feeds for each sensor and RSSI/Error LED
feeds = {
    ...
    '001ErrorLED': aio.feeds('001errorled')  # Error LED Feed
}
...
def handle_errors(status):
    ...
    try:
        if status == LoRa.STATUS_CRC_ERR or status == LoRa.STATUS_HEADER_ERR:
            # Light up the LED (send '1' to indicate an error)
            aio.send_data(feeds['001ErrorLED'].key, 1)
            print("Error occurred: Lighting up LED on Adafruit IO.")
        else:
            # Turn off the LED (send '0' when there is no error)
            aio.send_data(feeds['001ErrorLED'].key, 0)
    except Exception as e:
